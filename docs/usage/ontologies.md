...
- enter your API key and API endpoint in the respective fields (you can use GitHub Models for free by creating a GitHub PAT with permission to "models"),
- if you want to see what is being generated/extracted during the process set `enable-logging=True`,
...
- you can pass a list of entity types to the argument `entity_types` if you want to restrict the types of entities
  to be extracted. By default, it is set to `None` which means that the entity types will be generated by the LLM,
- by setting `extract_spl_triples` to `True` you will enable the extraction of s-p-l triples where l is a numeric 
   literal,
- if you want to have more control over the prompts, you can pass your own few-shot examples to the
  arguments: `examples_for_entity_extraction`, `examples_for_triples_extraction`, `examples_for_type_assertion`, 
  `examples_for_type_generation`, `examples_for_literal_extraction`, `examples_for_spl_triples_extraction`, 
  otherwise a default set of examples will be used. You can find the list of examples we use [here](https://github.com/dice-group/owlapy/blob/main/owlapy/ontogen/few_shot_examples.py).

After executing it over some text of your desire, the successful execution of the example code will generate a file 
named `generated_ontology.owl` in your local directory, which you can change by using the `save_path` argument.
