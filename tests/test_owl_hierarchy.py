...
from owlapy.owl_hierarchy import ClassHierarchy, DatatypePropertyHierarchy, ObjectPropertyHierarchy, _children_transitive, _reduce_transitive
...
class TestClassHierarchy(unittest.TestCase):
    ...
    def test_parents_of_bottom_transitive_returns_all_items(self):
        """parents(bottom, direct=False) takes the `yield from self.items()` branch."""
        hierarchy_data = [
            (self.person, [self.male]),
            (self.male, [])
        ]
        ...
        parents = list(hierarchy.parents(OWLNothing, direct=False))
        self.assertIn(self.person, parents)
        self.assertIn(self.male, parents)

    def test_children_of_top_transitive_returns_all_items(self):
        """children(top, direct=False) takes the `yield from self.items()` branch."""
        hierarchy_data = [
            (self.person, [self.male]),
            (self.male, [])
        ]
        ...
        children = list(hierarchy.children(OWLThing, direct=False))
        self.assertIn(self.person, children)
        self.assertIn(self.male, children)

    def test_is_parent_of_with_top_entity_and_unrelated_entities(self):
        hierarchy_data = [
            (self.person, [self.male]),
            (self.male, [self.father]),
            (self.female, [self.mother]),
        ...
        # OWLThing is a parent of everything (the `a == get_top_entity()` branch).
        self.assertTrue(hierarchy.is_parent_of(OWLThing, self.father))
        # Unrelated entities -> falls through to the final `return False`.
        self.assertFalse(hierarchy.is_parent_of(self.male, self.mother))

    def test_is_child_of_with_bottom_entity_and_unrelated_entities(self):
        hierarchy_data = [
            (self.person, [self.male]),
            (self.male, [self.father]),
            (self.female, [self.mother]),
        ...
        # OWLNothing is a child of everything (the `a == get_bottom_entity()` branch).
        self.assertTrue(hierarchy.is_child_of(OWLNothing, self.person))
        # Unrelated entities -> falls through to the final `return False`.
        self.assertFalse(hierarchy.is_child_of(self.mother, self.male))
