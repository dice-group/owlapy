...
- Python-side type checks for `owlapy.iri.IRI` (`__init__`/`create`, converting bare `-O`-strippable `assert`s to clear `TypeError`/`ValueError`), `owlapy.owl_individual.OWLAnonymousIndividual` (`node_id`), and `owlapy.owl_ontology.OWLOntologyID` (`ontology_iri`/`version_iri`) -- a broader sweep beyond #271's explicitly-tracked scope, covering the same silent-Python-success-then-opaque-JVM-failure pattern for the identity type nearly every OWL entity is built from (#271)
- Python-side type checks for `owlapy.swrl` (`Variable`/`IVariable`/`DVariable`, `ClassAtom`, `DataRangeAtom`, `ObjectPropertyAtom`, `DataPropertyAtom`, `SameAsAtom`, `DifferentFromAtom`, `BuiltInAtom`, `Rule`), the last unvalidated Python-Java mapping flagged in #271. Also fixed a latent bug in `Atom.from_string`'s built-in-predicate parsing branch where a misplaced `return` inside the arg-conversion loop silently left every argument but the first as a raw, unconverted string, and non-variable arguments were wrapped as `OWLNamedIndividual` instead of `OWLLiteral` (#271)
- Python-side type checks for `owlapy.owl_data_ranges`'s `OWLNaryDataRange`/`OWLDataComplementOf`, deliberately left out of the two PRs above: operands are validated against `OWLPropertyRange` (the common base of `OWLDataRange` and `OWLClassExpression`) rather than `OWLDataRange` itself, since `owlapy.utils.nnf.NNF` deliberately reuses these constructors to wrap data-side class expressions (e.g. `OWLDataSomeValuesFrom`) during negation. This closes out the remaining scope of #271 (all Python-Java mappings now have Python-side validation)
- Python-side type checks for the `owlapy.class_expression.restriction` constructors #272 missed: `OWLObjectSomeValuesFrom`/`OWLObjectAllValuesFrom` (property/filler), `OWLObjectHasSelf` (property), `OWLObjectHasValue` (property/individual), `OWLDataCardinalityRestriction` (`OWLDataMinCardinality`/`Max`/`Exact`, cardinality/property -- its `OWLObjectCardinalityRestriction` sibling already had this from #272), `OWLDataSomeValuesFrom`/`OWLDataAllValuesFrom` (property), `OWLDataHasValue` (property/value), `OWLObjectOneOf`/`OWLDataOneOf` (element type, now raising `TypeError` instead of a bare, `-O`-strippable `assert`), `OWLDatatypeRestriction` (datatype/facet restrictions), and `OWLFacetRestriction` (facet). `owlapy.owl_data_ranges`'s `OWLNaryDataRange`/`OWLDataComplementOf` were deliberately left unchanged -- they're reused by `owlapy.utils.nnf.NNF` to wrap data-side class expressions (e.g. `OWLDataSomeValuesFrom`) during negation, not just genuine `OWLDataRange` instances, so a strict `OWLDataRange` check there would break NNF (#271)
...
- `owlapy/utils.py` (1989 LOC) is now the package `owlapy/utils/`, split by concern into `similarity.py`, `length.py`, `ordering.py`, `nnf.py`, `simplify.py`, `signature.py`, and `cache.py`. `owlapy.utils`'s `__init__.py` re-exports every name that was genuinely part of `owlapy.utils`'s own API, plus `HasFiller`/`HasCardinality`/`HasOperands`/`OWLDataOneOf` since downstream code (e.g. Ontolearn's `tree_learner.py`) imports those from here specifically even though they're defined elsewhere; other names only reachable from the old flat module as a side effect of *its own* unrelated top-level imports (stdlib `typing` names, or other OWL entity/axiom classes) are not re-exported -- import those from their own module (IMPROVEMENTS.md 1.2)
...
### Fixed
- `SyncReasoner.equivalent_data_properties()` no longer always raises `AttributeError` (#237). It called `self._owlapi_reasoner.getEquivalentDataProperties(...)`, but that returns an OWLAPI `Node<OWLDataProperty>` (old-style API), not a `java.util.stream.Stream`, and `mapper.to_list()` unconditionally calls `.collect(Collectors.toList())`, which `Node` doesn't have. Now calls the Stream-returning `equivalentDataProperties(...)`, matching the correct pattern already used by `equivalent_object_properties()`.
