...
- `examples/runtime_benchmark_results.py` now prints timestamped progress (`[HH:MM:SS] <reasoner>: i/n done (Xs) -- <class expression>`) for each reasoner/class-expression pair instead of running silently -- some individual queries in the Carcinogenesis benchmark take tens of minutes (e.g. HermiT), during which the script previously gave no indication it was still working rather than hung.
