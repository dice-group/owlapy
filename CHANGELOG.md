...
- `owlapy.parallel_reasoner.BatchParallelReasoner`: a second parallel-reasoning strategy, sibling to `ParallelReasoner` (same worker-pool/JVM-per-worker lifecycle), that parallelizes across *many different* class-expression queries instead of one query's individuals -- each worker runs its own full, un-decomposed `SyncReasoner.instances(ce)` bulk call per expression via `instances_batch(expressions)`, preserving each reasoner's internal optimizations. Benchmarked the same way: 2.03x-5.04x faster than sequential `SyncReasoner` calls with HermiT (both a small and a 14K-individual ABox), but 0.16x-0.69x (slower) with Pellet on the same two datasets, since Pellet's bulk calls are already fast enough that worker-pool startup and JVM contention aren't amortized.

- `tests/test_parallel_reasoner_benchmark_regression.py`: a regression check (excluded from the default test run, same convention as `tests/test_z_do_last_ebr_retrieval.py` -- see CLAUDE.md) asserting `BatchParallelReasoner` beats sequential `SyncReasoner` calls by a conservative 1.2x margin with HermiT on a 40-expression subset of the checked-in Family corpus, guarding the `benchmarks/parallel_reasoner/` performance claim against regression.
